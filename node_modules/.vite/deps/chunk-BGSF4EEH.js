...
var Bone = class _Bone extends Node {
   ...
   */
  constructor(name48, skeleton, parentBone = null, localMatrix = null, restMatrix = null, bindMatrix = null, index = null) {
    super(name48, skeleton.getScene());
    this.name = name48;
    this.children = [];
    this.animations = [];
    this._index = null;
...
Mesh._instancedMeshFactory = (name48, mesh) => {
  const instance = new InstancedMesh(name48, mesh);
...
var InstancedMesh = class extends AbstractMesh {
   ...
   */
  constructor(name48, source) {
    super(name48, source.getScene());
   ...
   */
  createInstance(name48) {
    return this._sourceMesh.createInstance(name48);
  }
  /**
   ...
   * @returns the clone
   */
  clone(name48, newParent = null, doNotCloneChildren, newSourceMesh) {
    const result = (newSourceMesh || this._sourceMesh).createInstance(name48);
...
var AssetContainer = class extends AbstractScene {
  ...
  mergeAnimationsTo(scene = EngineStore.LastCreatedScene, animatables, targetConverter = null) {
    ...
    const _targetConverter = targetConverter ? targetConverter : (target) => {
      ...
      const name48 = target.name.split(".").join("").split("_primitive")[0];
      switch (targetProperty) {
        case "position":
        case "rotationQuaternion":
          node = scene.getTransformNodeByName(target.name) || scene.getTransformNodeByName(name48);
          break;
        case "influence":
          node = scene.getMorphTargetByName(target.name) || scene.getMorphTargetByName(name48);
          break;
        default:
          node = scene.getNodeByName(target.name) || scene.getNodeByName(name48);
      }
      return node;
    };
...
var Sound = class _Sound {
   ...
   */
  constructor(name48, urlOrArrayBuffer, scene, readyToPlayCallback = null, options) {
    var _a;
    this.autoplay = false;
    this._loop = false;
...
var Skeleton = class _Skeleton {
  ...
  toString(fullDetails) {
    ...
    if (fullDetails) {
      ret += ", Ranges: {";
      let first = true;
      for (const name48 in this._ranges) {
        if (first) {
          ret += ", ";
          first = false;
        }
   ...
   */
  getBoneIndexByName(name48) {
    for (let boneIndex = 0, cache = this.bones.length; boneIndex < cache; boneIndex++) {
      if (this.bones[boneIndex].name === name48) {
        return boneIndex;
      }
    }
    return -1;
   ...
   */
  createAnimationRange(name48, from, to) {
    if (!this._ranges[name48]) {
      this._ranges[name48] = new AnimationRange(name48, from, to);
      for (let i = 0, nBones = this.bones.length; i < nBones; i++) {
        if (this.bones[i].animations[0]) {
          this.bones[i].animations[0].createRange(name48, from, to);
        }
      }
    }
  }
   ...
   */
  deleteAnimationRange(name48, deleteFrames = true) {
    for (let i = 0, nBones = this.bones.length; i < nBones; i++) {
      if (this.bones[i].animations[0]) {
        this.bones[i].animations[0].deleteRange(name48, deleteFrames);
      }
    }
    this._ranges[name48] = null;
  }
  /**
   ...
   */
  getAnimationRange(name48) {
    return this._ranges[name48] || null;
  }
  /**
  ...
  getAnimationRanges() {
    ...
    let name48;
    for (name48 in this._ranges) {
      animationRanges.push(this._ranges[name48]);
    }
   ...
   */
  copyAnimationRange(source, name48, rescaleAsRequired = false) {
    if (this._ranges[name48] || !source.getAnimationRange(name48)) {
      return false;
    }
    let ret = true;
    ...
    for (i = 0, nBones = this.bones.length; i < nBones; i++) {
      ...
      if (sourceBone) {
        ret = ret && this.bones[i].copyAnimationRange(sourceBone, name48, frameOffset, rescaleAsRequired, skelDimensionsRatio);
      } else {
        Logger.Warn("copyAnimationRange: not same rig, missing source bone " + boneName);
        ret = false;
      }
    }
    const range = source.getAnimationRange(name48);
    if (range) {
      this._ranges[name48] = new AnimationRange(name48, range.from + frameOffset, range.to + frameOffset);
    }
    return ret;
  }
  /**
   ...
   */
  beginAnimation(name48, loop, speedRatio, onAnimationEnd) {
    const range = this.getAnimationRange(name48);
    if (!range) {
      return null;
    }
   ...
   */
  clone(name48, id) {
    const result = new _Skeleton(name48, id || name48, this._scene);
  ...
  serialize() {
    ...
    for (let index = 0; index < this.bones.length; index++) {
      ...
      for (const name48 in this._ranges) {
        const source = this._ranges[name48];
        if (!source) {
          continue;
        }
        const range = {};
...
var TargetCamera = class _TargetCamera extends Camera {
   ...
   */
  constructor(name48, position, scene, setActiveOnSceneIfNoneActive = true) {
    super(name48, position, scene, setActiveOnSceneIfNoneActive);
  ...
  createRigCamera(name48, cameraIndex) {
    if (this.cameraRigMode !== Camera.RIG_MODE_NONE) {
      const rigCamera = new _TargetCamera(name48, this.position.clone(), this.getScene());
...
var PassPostProcess = class _PassPostProcess extends PostProcess {
   ...
   */
  constructor(name48, options, camera = null, samplingMode, engine, reusable, textureType = 0, blockCompilation = false) {
    super(name48, "pass", null, null, options, camera, samplingMode, engine, reusable, void 0, textureType, void 0, null, blockCompilation);
  }
  /**
   * @internal
   */
...
var PassCubePostProcess = class _PassCubePostProcess extends PostProcess {
   ...
   */
  constructor(name48, options, camera = null, samplingMode, engine, reusable, textureType = 0, blockCompilation = false) {
    super(name48, "passCube", null, null, options, camera, samplingMode, engine, reusable, "#define POSITIVEX", textureType, void 0, null, blockCompilation);
    this._face = 0;
  }
  /**
   * @internal
...
var FileFaceOrientation = class {
  constructor(name48, worldAxisForNormal, worldAxisForFileX, worldAxisForFileY) {
    this.name = name48;
...
Node.AddNodeConstructor("Light_Type_1", (name48, scene) => {
  return () => new DirectionalLight(name48, Vector3.Zero(), scene);
});
var DirectionalLight = class extends ShadowLight {
   ...
   */
  constructor(name48, direction, scene) {
    super(name48, scene);
...
Node.AddNodeConstructor("Light_Type_2", (name48, scene) => {
  return () => new SpotLight(name48, Vector3.Zero(), Vector3.Zero(), 0, 0, scene);
});
var SpotLight = class _SpotLight extends ShadowLight {
   ...
   */
  constructor(name48, position, direction, angle, exponent, scene) {
    super(name48, scene);
    this._innerAngle = 0;
...
var PBRClearCoatConfiguration = class _PBRClearCoatConfiguration extends MaterialPluginBase {
  ...
  prepareDefinesBeforeAttributes(defines, scene) {
    ...
    if (this._isEnabled) {
      ...
      defines.CLEARCOAT_TEXTURE_ROUGHNESS_IDENTICAL = this._texture !== null && this._texture._texture === ((_a = this._textureRoughness) == null ? void 0 : _a._texture) && this._texture.checkTransformsAreIdentical(this._textureRoughness);
...
var PBRIridescenceConfiguration = class _PBRIridescenceConfiguration extends MaterialPluginBase {
  ...
  prepareDefinesBeforeAttributes(defines, scene) {
    ...
    if (this._isEnabled) {
      ...
      defines.IRIDESCENCE_USE_THICKNESS_FROM_MAINTEXTURE = this._texture !== null && this._texture._texture === ((_a = this._thicknessTexture) == null ? void 0 : _a._texture) && this._texture.checkTransformsAreIdentical(this._thicknessTexture);
      if (defines._areTexturesDirty) {
        if (scene.texturesEnabled) {
          if (this._texture && MaterialFlags.IridescenceTextureEnabled) {
...
var PBRSheenConfiguration = class extends MaterialPluginBase {
  ...
  prepareDefinesBeforeAttributes(defines, scene) {
    ...
    if (this._isEnabled) {
      ...
      defines.SHEEN_TEXTURE_ROUGHNESS_IDENTICAL = this._texture !== null && this._texture._texture === ((_a = this._textureRoughness) == null ? void 0 : _a._texture) && this._texture.checkTransformsAreIdentical(this._textureRoughness);
      if (defines._areTexturesDirty) {
        if (scene.texturesEnabled) {
          if (this._texture && MaterialFlags.SheenTextureEnabled) {
...
var shader4 = `uniform vec4 vEyePosition;uniform vec3 vReflectionColor;uniform vec4 vAlbedoColor;uniform vec4 vLightingIntensity;uniform vec4 vReflectivityColor;uniform vec4 vMetallicReflectanceFactors;uniform vec3 vEmissiveColor;uniform float visibility;uniform vec3 vAmbientColor;
#ifdef ALBEDO
...
var shader5 = `layout(std140,column_major) uniform;uniform Material {vec2 vAlbedoInfos;vec4 vAmbientInfos;vec2 vOpacityInfos;vec2 vEmissiveInfos;vec2 vLightmapInfos;vec3 vReflectivityInfos;vec2 vMicroSurfaceSamplerInfos;vec2 vReflectionInfos;vec2 vReflectionFilteringInfo;vec3 vReflectionPosition;vec3 vReflectionSize;vec3 vBumpInfos;mat4 albedoMatrix;mat4 ambientMatrix;mat4 opacityMatrix;mat4 emissiveMatrix;mat4 lightmapMatrix;mat4 reflectivityMatrix;mat4 microSurfaceSamplerMatrix;mat4 bumpMatrix;vec2 vTangentSpaceParams;mat4 reflectionMatrix;vec3 vReflectionColor;vec4 vAlbedoColor;vec4 vLightingIntensity;vec3 vReflectionMicrosurfaceInfos;float pointSize;vec4 vReflectivityColor;vec3 vEmissiveColor;vec3 vAmbientColor;vec2 vDebugMode;vec4 vMetallicReflectanceFactors;vec2 vMetallicReflectanceInfos;mat4 metallicReflectanceMatrix;vec2 vReflectanceInfos;mat4 reflectanceMatrix;vec3 vSphericalL00;vec3 vSphericalL1_1;vec3 vSphericalL10;vec3 vSphericalL11;vec3 vSphericalL2_2;vec3 vSphericalL2_1;vec3 vSphericalL20;vec3 vSphericalL21;vec3 vSphericalL22;vec3 vSphericalX;vec3 vSphericalY;vec3 vSphericalZ;vec3 vSphericalXX_ZZ;vec3 vSphericalYY_ZZ;vec3 vSphericalZZ;vec3 vSphericalXY;vec3 vSphericalYZ;vec3 vSphericalZX;
...
var shader6 = `varying vec3 vPositionW;
#if DEBUGMODE>0
...
var name7 = "samplerFragmentAlternateDeclaration";
...
var name8 = "pbrFragmentSamplersDeclaration";
var shader8 = `#include<samplerFragmentDeclaration>(_DEFINENAME_,ALBEDO,_VARYINGNAME_,Albedo,_SAMPLERNAME_,albedo)
...
var name9 = "subSurfaceScatteringFunctions";
var shader9 = `bool testLightingForSSS(float diffusionProfile)
...
var name10 = "importanceSampling";
var shader10 = `vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
var name11 = "pbrHelperFunctions";
var shader11 = `#define MINIMUMVARIANCE 0.0005
...
var name12 = "harmonicsFunctions";
var shader12 = `#ifdef USESPHERICALFROMREFLECTIONMAP
#ifdef SPHERICAL_HARMONICS
...
var name13 = "pbrDirectLightingSetupFunctions";
var shader13 = `struct preLightingInfo
...
var name14 = "pbrDirectLightingFalloffFunctions";
var shader14 = `float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
var name15 = "pbrBRDFFunctions";
var shader15 = `#define FRESNEL_MAXIMUM_ON_ROUGH 0.25
...
var name16 = "hdrFilteringFunctions";
var shader16 = `#ifdef NUM_SAMPLES
#if NUM_SAMPLES>0
...
var name17 = "pbrDirectLightingFunctions";
var shader17 = `#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
#ifdef SPECULARTERM
vec3 specular;
...
var name18 = "pbrIBLFunctions";
var shader18 = `#if defined(REFLECTION) || defined(SS_REFRACTION)
...
var name19 = "pbrBlockAlbedoOpacity";
var shader19 = `struct albedoOpacityOutParams
...
var name20 = "pbrBlockReflectivity";
var shader20 = `struct reflectivityOutParams
...
var name21 = "pbrBlockAmbientOcclusion";
var shader21 = `struct ambientOcclusionOutParams
{vec3 ambientOcclusionColor;
...
var name22 = "pbrBlockAlphaFresnel";
var shader22 = `#ifdef ALPHAFRESNEL
...
var name23 = "pbrBlockAnisotropic";
var shader23 = `#ifdef ANISOTROPIC
...
var name24 = "pbrBlockReflection";
var shader24 = `#ifdef REFLECTION
...
var name25 = "pbrBlockSheen";
var shader25 = `#ifdef SHEEN
...
var name26 = "pbrBlockClearcoat";
var shader26 = `struct clearcoatOutParams
...
var name27 = "pbrBlockIridescence";
var shader27 = `struct iridescenceOutParams
...
var name28 = "pbrBlockSubSurface";
var shader28 = `struct subSurfaceOutParams
...
var name29 = "pbrBlockNormalGeometric";
var shader29 = `vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
var name30 = "pbrBlockNormalFinal";
var shader30 = `#if defined(FORCENORMALFORWARD) && defined(NORMAL)
...
var name31 = "pbrBlockLightmapInit";
var shader31 = `#ifdef LIGHTMAP
...
var name32 = "pbrBlockGeometryInfo";
var shader32 = `float NdotVUnclamped=dot(normalW,viewDirectionW);float NdotV=absEps(NdotVUnclamped);float alphaG=convertRoughnessToAverageSlope(roughness);vec2 AARoughnessFactors=getAARoughnessFactors(normalW.xyz);
#ifdef SPECULARAA
...
var name33 = "pbrBlockReflectance0";
var shader33 = `float reflectance=max(max(reflectivityOut.surfaceReflectivityColor.r,reflectivityOut.surfaceReflectivityColor.g),reflectivityOut.surfaceReflectivityColor.b);vec3 specularEnvironmentR0=reflectivityOut.surfaceReflectivityColor.rgb;
#ifdef METALLICWORKFLOW
...
var name34 = "pbrBlockReflectance";
var shader34 = `#if defined(ENVIRONMENTBRDF) && !defined(REFLECTIONMAP_SKYBOX)
...
var name35 = "pbrBlockDirectLighting";
var shader35 = `vec3 diffuseBase=vec3(0.,0.,0.);
#ifdef SPECULARTERM
...
var name36 = "pbrBlockFinalLitComponents";
var shader36 = `aggShadow=aggShadow/numLights;
#if defined(ENVIRONMENTBRDF)
#ifdef MS_BRDF_ENERGY_CONSERVATION
...
var name37 = "pbrBlockFinalUnlitComponents";
var shader37 = `vec3 finalDiffuse=diffuseBase;finalDiffuse*=surfaceAlbedo.rgb;finalDiffuse=max(finalDiffuse,0.0);finalDiffuse*=vLightingIntensity.x;vec3 finalAmbient=vAmbientColor;finalAmbient*=surfaceAlbedo.rgb;vec3 finalEmissive=vEmissiveColor;
#ifdef EMISSIVE
...
var name38 = "pbrBlockFinalColorComposition";
var shader38 = `vec4 finalColor=vec4(
#ifndef UNLIT
#ifdef REFLECTION
finalIrradiance +
#endif
...
var name39 = "pbrBlockImageProcessing";
var shader39 = `#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
var name41 = "pbrPixelShader";
var shader41 = `#if defined(BUMP) || !defined(NORMAL) || defined(FORCENORMALFORWARD) || defined(SPECULARAA) || defined(CLEARCOAT_BUMP) || defined(ANISOTROPIC)
...
var name42 = "pbrVertexDeclaration";
var shader42 = `uniform mat4 view;uniform mat4 viewProjection;
#ifdef ALBEDO
...
var name43 = "pbrVertexShader";
var shader43 = `precision highp float;
      ...
      if (scene.texturesEnabled) {
        ...
        if (MaterialFlags.SpecularTextureEnabled) {
          ...
          if (this._metallicReflectanceTexture || this._reflectanceTexture) {
            const identicalTextures = this._metallicReflectanceTexture !== null && this._metallicReflectanceTexture._texture === ((_a = this._reflectanceTexture) == null ? void 0 : _a._texture) && this._metallicReflectanceTexture.checkTransformsAreIdentical(this._reflectanceTexture);
...
var PBRMaterial = class _PBRMaterial extends PBRBaseMaterial {
   ...
   */
  clone(name48, cloneTexturesOnlyOnce = true, rootUrl = "") {
    const clone = SerializationHelper.Clone(() => new _PBRMaterial(name48, this.getScene()), this, { cloneTexturesOnlyOnce });
    clone.id = name48;
    clone.name = name48;
...
Node.AddNodeConstructor("Light_Type_0", (name48, scene) => {
  return () => new PointLight(name48, Vector3.Zero(), scene);
});
var PointLight = class extends ShadowLight {
   ...
   * @param scene The scene the lights belongs to
   */
  constructor(name48, position, scene) {
    super(name48, scene);
...
var MorphTarget = class _MorphTarget {
   ...
   */
  constructor(name48, influence = 0, scene = null) {
    this.name = name48;
    this.animations = [];
   ...
   * @returns a new MorphTarget
   */
  static FromMesh(mesh, name48, influence) {
    if (!name48) {
      name48 = mesh.name;
    }
    const result = new _MorphTarget(name48, influence, mesh.getScene());
...
var name44 = "gaussianSplattingPixelShader";
var shader44 = `#include<clipPlaneFragmentDeclaration>
#include<logDepthDeclaration>
#include<fogFragmentDeclaration>
...
var name45 = "gaussianSplattingVertexDeclaration";
var shader45 = `uniform mat4 world;uniform mat4 view;uniform mat4 projection;
`;
...
var name46 = "gaussianSplattingUboDeclaration";
var shader46 = `#include<sceneUboDeclaration>
#include<meshUboDeclaration>
`;
ShaderStore.IncludesShadersStore[name46] = shader46;

// node_modules/@babylonjs/core/Shaders/gaussianSplatting.vertex.js
var name47 = "gaussianSplattingVertexShader";
var shader47 = `#include<__decl__gaussianSplattingVertex>
#ifdef LOGARITHMICDEPTH
#extension GL_EXT_frag_depth : enable
#endif
#include<clipPlaneVertexDeclaration>
...
var GaussianSplattingMaterial = class _GaussianSplattingMaterial extends PushMaterial {
   ...
   */
  clone(name48) {
    return SerializationHelper.Clone(() => new _GaussianSplattingMaterial(name48, this.getScene()), this);
  }
  /**
...
var DracoCompression = class _DracoCompression {
   ...
   */
  decodeMeshToGeometryAsync(name48, scene, data, attributes) {
    return this._decodeMeshAsync(data, attributes).then((meshData) => {
      const geometry = new Geometry(name48, scene);
      if (meshData.indices) {
  ...
  /** @internal */
  _decodeMeshToGeometryForGltfAsync(name48, scene, data, attributes, gltfNormalizedOverride) {
...
var GaussianSplattingMesh = class _GaussianSplattingMesh extends Mesh {
   ...
   */
  constructor(name48, url = null, scene = null) {
    super(name48, scene);
    this._vertexCount = 0;
    this._worker = null;
  ...
  static ConvertPLYToSplat(data) {
    ...
    for (const prop of filtered) {
      const [, type, name48] = prop.split(" ");
      properties.push({ name: name48, type, offset: rowOffset });
      if (!offsets[type])
...
var FlowGraphConnection = class {
  constructor(name48, _connectionType, _ownerBlock) {
...
var FlowGraphDataConnection = class extends FlowGraphConnection {
   ...
   * @param richType
   */
  constructor(name48, connectionType, ownerBlock, richType) {
    super(name48, connectionType, ownerBlock);
...
var FlowGraphBlock = class {
   ...
   */
  registerDataInput(name48, richType) {
    const input = new FlowGraphDataConnection(name48, FlowGraphConnectionType.Input, this, richType);
   ...
   * @returns the created connection
   */
  registerDataOutput(name48, richType) {
    const output = new FlowGraphDataConnection(name48, FlowGraphConnectionType.Output, this, richType);
   ...
   */
  getDataInput(name48) {
    return this.dataInputs.find((i) => i.name === name48);
  }
  /**
   ...
   * @returns the connection if it exists, undefined otherwise
   */
  getDataOutput(name48) {
    return this.dataOutputs.find((i) => i.name === name48);
  }
  /**
...
var FlowGraphExecutionBlock = class extends FlowGraphBlock {
  ...
  _registerSignalInput(name48) {
    const input = new FlowGraphSignalConnection(name48, FlowGraphConnectionType.Input, this);
    this.signalInputs.push(input);
    return input;
  }
  _registerSignalOutput(name48) {
    const output = new FlowGraphSignalConnection(name48, FlowGraphConnectionType.Output, this);
   ...
   */
  getSignalInput(name48) {
    return this.signalInputs.find((input) => input.name === name48);
  }
  /**
   ...
   * @returns if the input exists, the input. Otherwise, undefined.
   */
  getSignalOutput(name48) {
    return this.signalOutputs.find((output) => output.name === name48);
  }
  /**
   * Serializes this block
...
var FlowGraphContext = class {
   ...
   */
  hasVariable(name48) {
    return name48 in this._userVariables;
  }
  /**
  ...
  setVariable(name48, value) {
    this._userVariables[name48] = value;
  }
  /**
   ...
   */
  getVariable(name48) {
    return this._userVariables[name48];
  }
  /**
  ...
  _getUniqueIdPrefixedName(obj, name48) {
    return `${obj.uniqueId}_${name48}`;
  }
  /**
   ...
   * @param value
   */
  _setExecutionVariable(block, name48, value) {
    this._executionVariables[this._getUniqueIdPrefixedName(block, name48)] = value;
  }
  /**
   ...
   * @returns
   */
  _getExecutionVariable(block, name48, defaultValue) {
    if (this._hasExecutionVariable(block, name48)) {
      return this._executionVariables[this._getUniqueIdPrefixedName(block, name48)];
    } else {
      return defaultValue;
    }
  }
   ...
   * @param name
   */
  _deleteExecutionVariable(block, name48) {
    delete this._executionVariables[this._getUniqueIdPrefixedName(block, name48)];
  }
  /**
   ...
   * @returns
   */
  _hasExecutionVariable(block, name48) {
    return this._getUniqueIdPrefixedName(block, name48) in this._executionVariables;
  }
  /**
...
//# sourceMappingURL=chunk-BGSF4EEH.js.map
